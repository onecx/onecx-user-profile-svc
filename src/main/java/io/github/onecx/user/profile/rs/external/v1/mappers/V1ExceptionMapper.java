...
public abstract class V1ExceptionMapper {
    ...
    @LogService(log = false)
    public RestResponse<ProblemDetailResponseDTO> exception(DAOException de) {
        var e = exception(de.getMessageKey().name(), de.getMessage());
        return RestResponse.status(Response.Status.BAD_REQUEST, e);
    }
